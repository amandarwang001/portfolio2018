    ...
    <div class="container">
        <div class="row">
            <div class="col-12 text-center">
                <h1 class="pt-3">Dijatek Blog posts</h1>
            </div>
        </div>
...
//                    "<p><a href='./' class='btn btn-success'>Back to all posts</a></p>"
    ...
    $.ajax({
        data: "json",
        type: 'GET',
        url: 'https://blogs.regularjoeseo.com/wp-json/wp/v2/posts?per_page=20',
        success: function (data) {
            /*Build title column with clickable titles that load the content*/
            for (var i = 0; i < data.length; i++) {
                if (data[i].categories[0] == 8) {
                    console.log(data[i]);
                    ...
                    $(".blogtitles").append(
                        ...
                        "<a href='http://localhost:8888/php-template/public_html/blog/" + slug + "' class='card-link loadPosts faqlink' data-value='" + i + "'>" +
                        "<div class='card'>" +
                        ...
                        "<p class='card-text'><small>" + excerpt + "</small>" +
                        "</p>" +
                        "</div>" +
                        "</div>" +
                        "</a>" +
